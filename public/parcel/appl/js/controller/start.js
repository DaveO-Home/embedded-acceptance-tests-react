...
export default App.controllers.Start ||
    (App.controllers.Start = Object.assign({
        ...
        setLocation () {
            ...
            this.location = (hash === "#/!/contact" || hash === "#/contact" || hash === "#contact") ? this.location : hash;
        },
        location: "#/",
        index (options) {
        ...
        finish (options) {
            ...
            const mdFunction = data => {
                ...
                me.html = `${App.html} <div class="mx-auto" style="width: 90%">${marked.parse(data, markedOptions)}</div>`;
            };
            $.get(options.urlMd, mdFunction, "text")
            .fail(err => {
                console.warn("IT FAILED: " + options.urlMd, err);
            });
        },
        html: ""
    }, Base));
